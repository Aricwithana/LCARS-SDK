...
var LCARS = {
    ...
    */ 
     ...
     *          
     */	
	oncreateEvents: function(element, args){
        if(webviewInfo.input === 'desktop'){
            if(typeof args.click === 'function'){ $(element).click(args.click); if(args.type === 'radioButton' || args.type === 'checkboxButton'){$(element).click(preventLabelDoubleEvent);}}
            if(typeof args.mouseenter === 'function'){$(element).mouseenter(args.mouseenter); if(args.type === 'radioButton' || args.type === 'checkboxButton'){$(element).mouseenter(preventLabelDoubleEvent);}}
            if(typeof args.mouseleave === 'function'){$(element).mouseleave(args.mouseleave); if(args.type === 'radioButton' || args.type === 'checkboxButton'){$(element).mouseleave(preventLabelDoubleEvent);}}
            if(typeof args.hover === 'function'){$(element).hover(args.hover, args.hover); if(args.type === 'radioButton' || args.type === 'checkboxButton'){$(element).hover(preventLabelDoubleEvent, preventLabelDoubleEvent);}}
            if(typeof args.mousedown === 'function'){$(element).mousedown(args.mousedown); if(args.type === 'radioButton' || args.type === 'checkboxButton'){$(element).mousedown(preventLabelDoubleEvent);}}
            if(typeof args.mouseup === 'function'){$(element).mouseup(args.mouseup); if(args.type === 'radioButton' || args.type === 'checkboxButton'){$(element).mouseup(preventLabelDoubleEvent);}}           
        }else{                   
            if(typeof args.tap === 'function'){ $(element).tap(args.tap); if(args.type === 'radioButton' || args.type === 'checkboxButton'){$(element).tap(preventLabelDoubleEvent);}}
            if(typeof args.singleTap === 'function'){$(element).singleTap(args.singleTap); if(args.type === 'radioButton' || args.type === 'checkboxButton'){$(element).singleTap(preventLabelDoubleEvent);}}
            if(typeof args.doubleTap === 'function'){$(element).doubleTap(args.doubleTap); if(args.type === 'radioButton' || args.type === 'checkboxButton'){$(element).doubleTap(preventLabelDoubleEvent);}}
            if(typeof args.longTap === 'function'){$(element).longTap(args.longTap); if(args.type === 'radioButton' || args.type === 'checkboxButton'){$(element).longTap(preventLabelDoubleEvent);}}                    
            if(typeof args.swipe === 'function'){ $(element).swipe(args.swipe); if(args.type === 'radioButton' || args.type === 'checkboxButton'){$(element).swipe(preventLabelDoubleEvent);}}
            if(typeof args.swipeLeft === 'function'){$(element).swipeLeft(args.swipeLeft); if(args.type === 'radioButton' || args.type === 'checkboxButton'){$(element).swipeLeft(preventLabelDoubleEvent);}}
            if(typeof args.swipeRight === 'function'){$(element).swipeRight(args.swipeRight); if(args.type === 'radioButton' || args.type === 'checkboxButton'){$(element).swipeRight(preventLabelDoubleEvent);}}
            if(typeof args.swipeUp === 'function'){$(element).swipeUp(args.swipeUp); if(args.type === 'radioButton' || args.type === 'checkboxButton'){$(element).swipeUp(preventLabelDoubleEvent);}}
            if(typeof args.swipeDown === 'function'){$(element).swipeDown(args.swipeDown); if(args.type === 'radioButton' || args.type === 'checkboxButton'){$(element).swipeDown(preventLabelDoubleEvent);}}                                    
        }    
    },
     ...
     *          or another.
     *  @setGet - Allows $.objectSettings(); to interact with the LCARS.fn
     *            without creating new/overriding incorrectly a definition.
     *  !note - If functionality needs to be added custom for a project
     *          override 'LCARS.setObjectSettings' with your own copy
     *          of this function with your additional parameters.
        ...
        if(setGet === true){
            ...
            for (var prop in args) {
              if(args.hasOwnProperty(prop)){
                if(targetObj[prop]){$(element).removeAttr(prop).removeClass(targetObj[prop]);}
     ...
     *  &info - This maintains the base default of settings the SDK utilizes for 
     *          element creation and features.
     *  !note - If functionality needs to be added custom for a project
     *          override 'LCARS.setObjectSettings' with your own copy
     *          of this function with your additional parameters.
     *          DO NOT EDIT THIS, WILL BE OVERWRITTEN WHEN SDK IS UPGRADED.
     */	
    setObjectSettings: function(element, args, setGet){
        args = LCARS.objectDefinition(args, setGet, element);
        
        if(args.id){$(element).attr('id', args.id);}
        if(args.type){$(element).addClass(args.type);}
        if(args.version){$(element).addClass(args.version);}  
        if(args.label){$(element).not('.complexButton').attr('data-label', args.label);}		 
        if(args.altLabel){$(element).not('.complexButton').attr('data-altLabel', args.altLabel);}
        if(args.color){$(element).addClass(args.color);}
        if(args.attrs){$(args.attrs).each(function(){$(element).attr(this.attr, this.value);});}
        if(args.size){$(element).addClass(args.size);}
        if(args.state){$(element).addClass(args.state);}
        if(args.noTransition === true){$(element).addClass('noTransition');}
        if(args.noEvent === true){$(element).addClass('noEvent');}
        if(args.hidden === true){$(element).addClass('hidden');}
        if(args.fade === true){$(element).addClass('fade');}
        if(args.class){$(element).addClass(args.class);} 
        if(args.flex){$(element).addClass('flex'+args.flex);}
        if(args.flexC){$(element).addClass('flexc'+args.flexC);}
        if(args.readOnly === true){$(element).attr('readonly', true);}	
        if(args.name){$(element).attr('name', args.name);}
        if(args.checked === true){$(element).prop('checked', true);}
        if(args.href){$(element).attr('href', args.href);}
        if(args.src){$(element).attr('src', args.src);}
        ...
        if(typeof args.children === 'string'){$(element).append(args.children);}                
        
        if(webviewInfo.input === 'desktop'){      
            if((args.click || args.mouseenter || args.mouseleave || args.hover || args.mousedown || args.mouseup) !== undefined){LCARS.oncreateEvents(element, args);}                 
        }else{
            if((args.tap || args.singleTap || args.doubleTap || args.longTap || args.swipe || args.swipeLeft || args.swipeRight || args.swipeUp || args.swipeDown) !== undefined){LCARS.oncreateEvents(element, args);}
        }                
        return element;
    },
	...
	create: {
        ...
        complexButton:function(args){
            ...
            $(args.template).each(function(){
                if(this.type == 'button'){
                    ...
                    if(args.altLabel !== undefined){this.label = args.altLabel;}
                }                 
        ...
        numericButton:function(args){
            ...
            $(args.template).each(function(){
                if(this.type == 'numericBlock'){
                    var elementChild = LCARS.create.block({type:'block', class:'numericBlock'});
                    if(args.value !== undefined){$(elementChild).text(args.value);}
                }else{
                    if(this.type == 'button'){
                        if(args.label !== undefined){this.label = args.label;}
                        if(args.altLabel !== undefined){this.label = args.altLabel;}
        ...
        textInput:function(args){
            ...
            element = LCARS.setObjectSettings(element, args);
            if(args.text){$(element).val(args.text);}
            return element;	        
        },
        ...
        dialog:function(args){
            var element = LCARS.create[args.template.type](args.template);
            if(args.headerTitle !== undefined){$(element).find('.header').children('.title').text(args.headerTitle);}	
            if(args.footerTitle !== undefined){$(element).find('.footer').children('.title').text(args.footerTitle);}
            element = LCARS.setObjectSettings(element, args);
            return element;	        
        }
	}   
 ...
 */	
$.fn.numericBlockValue = function(value, success){  
    var elemID = $(this).attr('id');
    var objectDef = allObjects[elemID]
    if(!value || typeof value === 'function'){
        if(objectDef.value){
            var returnVal = objectDef.value;
        ...
        this.each(function(){
            if(value === 'remove'){
                ...
                delete objectDef.value;
            }else{   
                this.children('.numericBlock').text(value);
                objectDef.value = value;
            }
        });
 ...
 */	    
$.fn.objectSettings = function(setting, value, success){		
    var elemID = $(this).attr('id');
    var objectDef = allObjects[elemID]
    if(!value || typeof value === 'function'){ 
        if(objectDef[setting]){
            if(typeof success === 'function'){success();}
            return objectDef[setting];
        }else{
        ...
        this.each(function(){
            if(value === 'remove'){
                $(this).removeAttr(setting).removeClass(value);
                delete objectDef[value];
            }else if(typeof setting === 'object'){
                LCARS.setObjectSettings($(this), setting, true);
            }else if(typeof value === 'string'){   
                var settings = {}
                settings[setting] = value;    
                LCARS.setObjectSettings($(this), settings, true);
            }
        });
        if(typeof success === 'function'){success();}
        return this;
...
$.fn.objectColor = function(action, success){   
    ...
    if(typeof action === 'function' || !action){
        ...
        this.each(function(){
            var newColor = LCARS.colorGen(action);
            objectDef.color = newColor;
            $(this).removeClass(objectDef.color).addClass(newColor);
        });
        if(typeof success === 'function'){success();}
        return this;   
...
*/
$(document).on('touchstart', '.ie.touch .complexButton:not(.disabled):not(.noEvent)', function(){
    $(this).addClass('active');
});

$(document).on('touchend', '.ie .touch .complexButton.active', function(e){      
    $(this).removeClass('active'); 
});

$(document).on('touchcancel', '.ie.touch .complexButton', function(e){      
    $(this).removeClass('active'); 
});

$(document).on('mousedown', '.ie .complexButton:not(.disabled):not(.noEvent), .ie .elbow:not(.disabled):not(.noEvent)', function(){
    $(this).addClass('active');
});

$(document).on('mouseup', '.ie .complexButton.active, .ie .elbow.active', function(e){      
    $(this).removeClass('active'); 
});
